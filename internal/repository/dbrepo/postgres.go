...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	query := `select r.id, r.room_name from rooms r where r.id not in
				(select room_id from room_restrictions rr where $1 < end_date and $2 > start_date)`
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date,
				...
				left join rooms rm on (r.room_id = rm.id) order by r.start_date asc`
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date,
				...
				left join rooms rm on (r.room_id = rm.id) where processed = 0 order by r.start_date asc`
